	...
	<div id="entry_area">
		...
		<div id="content_area">
			<form action="end" method="post">
				...
				<span><input type="radio" name="destination" checked>직접입력</span>
				<c:forEach var="des" items="${des}" varStatus="index">
					<span>
						<input type="radio" name="destination" id="des_${index.count}">${des.dname}
					</span>
					<input type="hidden" id="des_${index.count}_dname" value="${des.dname}">
					<input type="hidden" id="des_${index.count}_name" value="${des.name}">
					<input type="hidden" id="des_${index.count}_phone" value="${des.phone}">
					<input type="hidden" id="des_${index.count}_address" value="${des.address}">
					<input type="hidden" id="des_${index.count}_dmemo" value="${des.dmemo}">
				</c:forEach>
				<br>
				<input type="text" name="dname" id="dname" placeholder="배송지명" required><br>
				...
				<input type="text" name="dmemo" id="dmemo" placeholder="배송메모" required><br>
				<br>
				<c:forEach var="cart" items="${cart}">
					...
					<table border="1" style="display: inline-block; border-collapse: collapse;">
						<tr>
							<td>
								<a href="/board/detail?pno=${cart.pno}">
									<img src="/display?fileName=${cart.ivo.fullPath}" style="width: 200px; height: 200px;">
								</a>
							</td>
